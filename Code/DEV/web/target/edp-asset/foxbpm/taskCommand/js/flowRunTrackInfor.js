		...
		if (runningTrackLength != 0 && runningTrackIndex < runningTrackLength) {
			...
			for (var j = 0; j < rectAttributes.length; j++) {
				...
				if (rectAttribute.name == "stroke") {
					rectAttribute.value = backUpRunningTrackColorDictionary[currentRunningTrack.nodeId];
				}
				if (rectAttribute.name == "stroke-width") {
					rectAttribute.value = backUpRunningTrackWidthDictionary[currentRunningTrack.nodeId];
				}
			}
		...
		$.ajax({
			...
			success : function(msg) {// msg为返回的数据，在这里做数据绑定
				...
				if (runningTrackInfor) {
					$("#loadTips").hide();
					table_thead = $("<thead>");
					table_thead.append("<th style='width: 180px'>轨迹节点</th>");
//					table_thead.append("<th style='width: 300px'>轨迹令牌编号 / 轨迹父令牌编号</th>");
					table_thead.append("<th style='width: 150px'>运行时间</th>");
					table_thead.append("<th style='width: 130px'>事件名称</th>");
					table_thead.append("<th style='width: 100px'>处理者</th>");
//					table_thead.append("<th style='width: 300px'>流程实例编号</th>");
					table_thead.append("</thead>");
					table.append(table_thead);

					$.each(runningTrackInfor, function(i, row) {
						table_tr = $("<tr " + (i % 2 == 0 ? "class='gray'" : "") + ">");
						table_tr.append("<td>" + row.nodeId + (!row.nodeName ? '' : " / " + row.nodeName) + "</td>");
//						table_tr.append("<td>" + row.tokenId + (!row.parentTokenId ? '' : " </br> " + row.parentTokenId) + "</td>");
		...
		if (index != 0) {
			...
			if (nodeId) {
				...
				for (var j = 0; j < rectAttributes.length; j++) {
					var rectAttribute = rectAttributes[j];
					if (rectAttribute.name == "stroke") {
						rectAttribute.value = backUpRunningTrackColorDictionary[runningTrackInfor[runningTrackIndex - 1].nodeId];
					}
					if (rectAttribute.name == "stroke-width") {
						rectAttribute.value = backUpRunningTrackWidthDictionary[runningTrackInfor[runningTrackIndex - 1].nodeId];
					}
		...
		if (index != 0) {
			...
			if (markerPath) {
				...
				for (var j = 0; j < rectAttributes.length; j++) {
					var rectAttribute = rectAttributes[j];
					if (rectAttribute.name == "stroke") {
						rectAttribute.value = backUpRunningTrackColorDictionary[markerPathId];
					}
					if (rectAttribute.name == "stroke-width") {
						rectAttribute.value = backUpRunningTrackWidthDictionary[markerPathId];
					}
					...
					for (var j = 0; j < rectAttributes.length; j++) {
						...
						if (rectAttribute.name == "stroke") {
							if (tempNodeID != currentRunningTrack.nodeId) {
								backUpRunningTrackColorDictionary[currentRunningTrack.nodeId] = rectAttribute.value;
								rectAttribute.value = RUNNING_TRACK_COLOR;
							}
						}
						if (rectAttribute.name == "stroke-width") {
							if (tempNodeID != currentRunningTrack.nodeId) {
								backUpRunningTrackWidthDictionary[currentRunningTrack.nodeId] = rectAttribute.value;
								rectAttribute.value = RUNNING_TRACK_WIDTH;
							}
						}
					}
					...
					for (var j = 0; j < rectAttributes.length; j++) {
						...
						if (rectAttribute.name == "stroke") {
							if (tempNodeID != markerPathId) {
								backUpRunningTrackColorDictionary[markerPathId] = rectAttribute.value; 
								rectAttribute.value = RUNNING_TRACK_COLOR;
							}
						}
						if (rectAttribute.name == "stroke-width") {
							if (tempNodeID != markerPathId) {
								backUpRunningTrackWidthDictionary[markerPathId] = rectAttribute.value; 
								rectAttribute.value = RUNNING_TRACK_WIDTH;
							}
						}
					}
