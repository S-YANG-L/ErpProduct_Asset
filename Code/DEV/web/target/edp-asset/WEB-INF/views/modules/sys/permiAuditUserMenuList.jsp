        ...
        <div class="row">
        	<div class="col-xs-9" style="padding-left:0">
	            <form:form id="searchForm" modelAttribute="user"
	                       action="${ctx}/sys/user/listData" method="post" class="form-inline"
	                       data-page-no="${page.pageNo}" data-page-size="${page.pageSize}"
	                       data-order-by="${page.orderBy}">
	                <div class="form-group">
	                    <label class="control-label"style="margin-left: 20px">登录账号：</label>
            ...
            </div><div class="col-xs-3" style="padding:0">
            ...
            <div id="search">
                <input id="key" type="text" placeholder="请输入搜索条件" class="span8"
                       style="margin:0px 0px 0px 20px; width: 150px; height: 29px; vertical-align: middle;"/>
...
<script type="text/javascript">
    ...
    $('#dataGrid').dataGrid({
        ...
        // 设置数据表格列
        columnModel: [
            { header: '登录账号', name: 'userCode', index: 'user_code', width: 200, align: "left"  },
            { header: '用户名称', name: 'userName', index: 'user_name', width: 200, align: "left"  },
            { header: '组织机构', name: 'refObj.officeName', index: 'd.office_name', width: 200, align: "left"  }
        ],
        ajaxSuccess: function (data) { // 加载成功后执行方法
        },
        onSelectRow : function(id){
            var url="${ctx}/sys/safetyAudit/permiAudit/listUserMenuData?userCode="+id;
            $.post(url, function(data){
                if(data.length > 0){
                    var str='[';
                    jQuery.each(data, function(i, item) {
                        str = str + "{ id:" + item.menuCode + ", pId:" + (item.parentCode!=null ? item.parentCode : 0) + ", pIds:\"" + item.parentCodes + "\" , name:\"" + item.name + "\"},";
                    });
                    str = str.substring(0, str.length - 1);
                    str = str + "]";
                    var oStr = eval("(" + str + ")");
                    tree = jQuery.fn.zTree.init(jQuery("#tree"), setting, oStr);
                    // 默认展开一级节点
                    ...
                    tree = jQuery.fn.zTree.init(jQuery("#tree"), setting, []);
                }
            },"json");
        }
    });
