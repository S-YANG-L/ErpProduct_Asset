...
for /f %%i in (compressor.tmp) do (
	...
	if !last! neq min.js (
		echo in %%i
		java -jar %~dp0\yuicompressor-2.4.7.jar --line-break 30000 --type js --charset utf-8 -o !first!.min.js %%i
...
for /f %%i in (compressor.tmp) do (
	...
	if !last! neq min.css (
		echo in %%i
		java -jar %~dp0\yuicompressor-2.4.7.jar --line-break 30000 --type css --charset utf-8 -o !first!.min.css %%i
