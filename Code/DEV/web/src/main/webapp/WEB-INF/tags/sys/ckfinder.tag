...
<%@ include file="/WEB-INF/views/include/taglib.jsp"%><%-- 抛弃组件，建议使用，fileupload.tag --%>
...
<ol id="${input}Preview" style="padding-left:18px;"></ol>
<c:if test="${!readonly}">
	<span id="${input}Btns"><a href="javascript:" onclick="${input}FinderOpen();" class="btn default" style="margin-left:18px;">${selectMultiple?'添加':'选择'}</a>&nbsp;
	<a href="javascript:" onclick="${input}DelAll();" class="btn default">清除</a></span>
</c:if>
	...
	function ${input}Preview(){
		...
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){//<c:if test="${type eq 'thumb' || type eq 'images'}">//
				li = "<li><img src=\""+urls[i]+"\" url=\""+urls[i]+"\" style=\"max-width:${empty maxWidth ? 150 : maxWidth}px;max-height:${empty maxHeight ? 150 : maxHeight}px;_height:${empty maxHeight ? 150 : maxHeight}px;border:0;padding:3px;\">";//</c:if><c:if test="${type ne 'thumb' && type ne 'images'}">//
