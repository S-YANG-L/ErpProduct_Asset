...
<div class="container process" id="processInfo">
    <div class="discussion-timeline js-quote-selection-container" id="taskInforEle">
    	<c:choose>
    		<c:when test="${empty flowBusinessApprovalList }">会审人员列表为空</c:when>
    		<c:otherwise><div style="text-align:right;"><c:choose>
		                                	<c:when test="${jointStatus eq '1'}"><span style="margin-right:20px;">【审批中】</span></c:when>
		                                	<c:when test="${jointStatus eq '2'}"><span style="color:green;margin-right:20px;">【审批通过】</span></c:when>
		                                	<c:when test="${jointStatus eq '3'}"><span style="color:red;margin-right:20px;">【审批不通过】</span></c:when>
		                                	<c:otherwise></c:otherwise>
		                                </c:choose>待审批人数：<span style="margin-right:20px;">${todoCount }</span>通过人数：<span style="color:green;margin-right:20px;">${passCount }</span>不通过人数：<span style="color:red;margin-right:20px;">${notPassCount }</span></div></c:otherwise>
    	</c:choose>
		                    ...
		                    <div class="timeline-comment-header">
		                        ...
		                        <div class="timeline-comment-header-text">
		                            <strong>
		                                ...
		                                &nbsp;
		                                <c:choose>
		                                	<c:when test="${flowBusinessApproval.approvalStatus eq '1'}">待审批</c:when>
		                                	<c:when test="${flowBusinessApproval.approvalStatus eq '2'}">审批通过</c:when>
		                                	<c:when test="${flowBusinessApproval.approvalStatus eq '3'}">审批不通过</c:when>
		                                	<c:otherwise></c:otherwise>
		                                </c:choose>
		                            </strong>
		                            <strong>
