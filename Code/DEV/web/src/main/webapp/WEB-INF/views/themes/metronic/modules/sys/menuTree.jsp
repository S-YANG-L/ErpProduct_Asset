	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$(".accordion-body a").click(function(){
				$("#menu-${param.parentCode} li").removeClass("active");
				$("#menu-${param.parentCode} li i").removeClass("icon-white");
...
<body>
	...
	<div class="accordion" id="menu-${not empty param.parentCode ? param.parentCode : '0'}">
		<c:set var="firstMenu" value="true"/>
		<c:forEach items="${menuList}" var="menu" varStatus="idxStatus">
			<c:if test="${menu.parent.id eq (not empty param.parentCode ? param.parentCode : '0') && menu.isShow eq '1'}">
				<div class="accordion-group">
				    <div class="accordion-heading">
				    	<a class="accordion-toggle" data-toggle="collapse" data-parent="#menu-${param.parentCode}" data-href="#collapse-${menu.id}" href="#collapse-${menu.id}" title="${menu.remarks}"><i class="icon-chevron-${not empty firstMenu && firstMenu ? 'down' : 'right'}"></i>&nbsp;${menu.name}</a>
				    </div>
