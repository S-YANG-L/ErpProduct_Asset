						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<sys:treeselect id="company" name="company.companyCode" value="${baoxiao.company.companyCode}" labelName="company.companyName" labelValue="${baoxiao.company.companyName}"
									title="公司" url="/sys/company/treeData" cssClass="" allowClear="true" notAllowSelectParent="true" disabled="disabled"/>
							</div>
						</div>
					</div>
					<div class="col-xs-6">
						<div class="form-group">
							...
							<div class="col-sm-8">
								<sys:treeselect id="office" name="office.officeCode" value="${baoxiao.office.officeCode}" labelName="office.officeName" labelValue="${baoxiao.office.officeName}"
									title="部门" url="/sys/office/treeData" cssClass="required" allowClear="true" notAllowSelectParent="true" disabled="disabled"/>
							</div>
						</div>
					</div>
				</div>
...
<script type="text/javascript">
	...
	$('#dataGrid').dataGrid({
		// 当前页签编号
			...
			columnModel: [
				{header:'费用项目', name:'baoxiaoType',  width:200,editable: false }, 
				{header:'项目名称', name:'projCode',  width:200, editable: false },
	            {header:'金额', name:'jine', width:200, align:"right", editable: true ,
				 ...
				 {header:'摘要', name:'zhaiyao', width:300, editable: false }
			],
			...
			ajaxSuccess: function(data){ // 加载成功后执行方法
				 if(data){//判断是否有数据（判断的方法可以跟踪console.log(data)来决定）
						tatalApplyMoney=0;
						// 初始化后编辑所有行
						var dg = $("#dataGrid");
