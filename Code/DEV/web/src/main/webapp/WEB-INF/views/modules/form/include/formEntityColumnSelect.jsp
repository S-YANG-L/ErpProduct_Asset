...
<div class="panel panel-default" id="dropdownID">
    <div class="panel-heading">
        <h3 class="panel-title">下拉框</h3>
    </div>
    <div class="panel-body">
    	<div class="row">
    		<div class="col-xs-12">
	        	<div class="form-group">
	        		<label class="control-label col-sm-2" title="指定下拉框的数据来源：
系统字典：从系统字典中进行选择；
用户字典：从用户字典中进行选择；
...
自定义SQL：输入sql语句，或者从脚本管理中选择已有sql语句；">
    	...
    	<div class="row">
    		<div class="col-xs-6">
				<div class="form-group">
					<label class="control-label col-sm-4" title="勾选则下拉框为多选">
	        		是否多选：<i class="fa icon-question"></i></label>
					<div class="col-sm-8">
						<c:choose>
							<c:when test="${empty (formEntityColumn.showOptions)}">
								<input type="checkbox"  value="0" name="optionMap['selectorMultipleChoice']"/>是
							</c:when>
							<c:otherwise>
								<!-- 设置标记位 -->
								...
								<c:forEach items="${formEntityColumn.optionMap}" var="map">
									<c:if test="${map.key=='selectorMultipleChoice'}">
										<input type="checkbox"  id="selmcheckbox" onchange="setCheckd('selmcheckbox');" name="optionMap['selectorMultipleChoice']" value="0" <c:if test="${map.value=='0'}"> checked="checked" </c:if> />是
										<c:set var="flag" value="1"></c:set>
										<input type="checkbox" id="noselmcheckbox" name="optionMap['selectorMultipleChoice']" value="" class="hide"/>
									</c:if>
								</c:forEach>
        ...
        <div class="row hide" id="dictRow">
        	<div class="col-xs-6">
	        	<div class="form-group">
	        		...
	        		<span class="required">*</span>系统字典：<i class="fa icon-question hide"></i></label>
					<div class="col-sm-8">
        ...
        <div class="row hide" id="userDictRow">
        	<div class="col-xs-6">
	        	<div class="form-group">
	        		<label class="control-label col-sm-4" title="">
	        		<span class="required">*</span>用戶字典：<i class="fa icon-question hide"></i></label>
					<div class="col-sm-8">
						<select name="optionMap['userDictTypeName']"  class="form-control" id="optionMapUserSelect">
							<option value="">请选择</option>
							<c:forEach items="${userDictTypeList}" var="dict">
								<option value="${dict.dictTypeCode}" <c:if test="${dict.dictTypeCode eq formEntityColumn.optionMap.userDictTypeName}"> selected="selected"</c:if> >${dict.dictTypeName}</option>
							</c:forEach>
						</select>
					</div>
	        	</div>
...
<script type="text/javascript">
	...
	if (obj == '1') {
		...
		//显示用户字典类型
		$("#userDictRow").removeClass("hide");
		$("#optionMapUserSelect").addClass("required");
	}
	...
	function radioChange(v) {
		if (v == '1') {
			...
			$("#optionMapSelect").addClass("required");
			//隐藏用户字典类型
			$("#userDictRow").addClass("hide");
			$("#optionMapUserSelect").removeClass("required");
			$("#optionMapUserSelect").select2('val', '');
			//隐藏自定义SQL
			$("#sqlRow").addClass("hide");
			$("#sqlInp").removeClass("required");
			$("#sqlInp").val('');
