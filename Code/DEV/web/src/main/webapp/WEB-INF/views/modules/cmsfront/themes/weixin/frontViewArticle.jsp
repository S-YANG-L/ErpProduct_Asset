...
<body>
   ...
   <div class="men_pho" id="men_pho_id" style="	z-index:10003;display:none"></div>
	<script type="text/javascript">
		...
		function clickBack(){
			window.location.href = "${ctx}/list-${category.categoryCode}${urlSuffix}";
		...
		<div id="win">
			<ul class="dropdown">
				<c:forEach items="${fnc:getMainNavList(site.siteCode)}" var="category" varStatus="status">
					<li><a href="${category.url}" ><span>${category.categoryName}</span></a></li>
			   	</c:forEach>
				<div class="clr"></div>
			</ul>
		</div>
		...
		<div class="row-fluid">
			<div class="span12">
				<div>
					...
					<span id="wz_date">发布者：${article.createBy.userName} &nbsp; 点击数：${article.hits} &nbsp; 发布时间：<fmt:formatDate value="${article.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/> &nbsp; 更新时间：<fmt:formatDate value="${article.updateDate}" pattern="yyyy-MM-dd HH:mm:ss"/></span>
				...
				<div class="seemore">
					<a class="no_border"  href="${ctx}/index-${site.siteCode}${fns:getUrlSuffix()}" >查看更多</a>
				</div>
