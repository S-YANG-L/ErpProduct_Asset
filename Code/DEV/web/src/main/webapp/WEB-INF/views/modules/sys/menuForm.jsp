		...
		<form:form id="inputForm" modelAttribute="menu" action="${ctx}/sys/menu/save" method="post" class="form-horizontal">
			...
			<div class="form-body">
				...
				<%-- <div class="row">
					...
					<div class="col-xs-6">
						<div class="form-group">
							<label class="control-label col-sm-4"><span class="required">*</span> 菜单编号：</label>
							<div class="col-sm-8">
								<form:hidden path="isNewRecord"/>
								<form:input path="menuCode" htmlEscape="false" maxlength="30" readonly="${!menu.isNewRecord}" class="form-control abc required"/>
							</div>
						</div>
					</div> --%>
					<div class="col-xs-6">
...
// 						contentWindow.dataGrid.refreshTree(1, $('#parentCode').val());
						contentWindow.$('#dataGrid').dataGrid('refreshTreeChildren',
								$('#parentCode').val(), ${menu.isNewRecord});
					});
				}
			}, "json");
	    }
	...
	function refreshType(val){
		if (val == '2'){
			...
			//$('input[name=isShow][value=0]').attr('checked', 'checked');
			//$.uniform.update('input[name=isShow]');
		}else{
			$('.isPerm').hide();
			$('.isMenu').show();
			//$('input[name=isShow]').removeAttr('checked');
			//$('input[name=isShow][value=1]').attr('checked', 'checked');
			//$.uniform.update('input[name=isShow]');
		}
	}
</script>