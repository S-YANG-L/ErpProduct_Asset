...
<html>
<head>
	<title>全站搜索 - Powered By Heracles</title>
			...
			<dl class="search">
				<c:if test="${empty t || t eq 'article'}">
					<c:forEach items="${page.list}" var="article">
						<dt><a href="${ctx}/view-${article.categoryCode}-${article.id}${urlSuffix}" target="_blank">${article.title}</a></dt>
						<dd>${article.content}
							<div class="info">
								发布者：${article.createBy} &nbsp; 点击数：${article.hits} &nbsp; 
								发布时间：${article.createDateStr} &nbsp; 更新时间：${article.updateDateStr} &nbsp;
								<a href="${ctx}/view-${article.categoryCode}-${article.id}${urlSuffix}" target="_blank">查看全文</a><br/>
							</div>
						</dd>
					</c:forEach>
				</c:if>
