...
<sys:header title="${fns:text('用户管理 ')}- ${user.isNewRecord ? fns:text('新增') : fns:text('编辑')}${fns:text('用户')}" extLibs="${extLibs}" />
<div class="portlet box ${boxStyleColor} tabbable autoHeight">
	<div class="portlet-title">
		<div class="caption">
			<i class="fa fa-user"></i>${user.isNewRecord ? fns:text('新增') : fns:text('编辑')}${fns:text('用户')}
		</div>
	</div>
	<div class="portlet-body form">
		<form:form id="inputForm" modelAttribute="user" action="${ctx}/sys/user/save?op=${param.op}" method="post" class="form-horizontal">
...
<script>
	...
	jQuery(document).ready(function() {  
		...
		$("#inputForm").validate({
// 			rules: {
// 				userCode: {remote: "${ctx}/sys/user/checkUserCode?oldUserCode=" + encodeURIComponent("${user.userCode}")},
// 			},
// 			messages: {
