...
<sys:header title="员工查询" extLibs="${extLibs}"  extHead="${extHead}"/>
<div class="portlet box">
	<div class="portlet-body list">
		<form:form id="searchForm" modelAttribute="employee" action="${ctx}/sys/employee/listData?isAll=${param.isAll}" method="post" class="breadcrumb form-inline"
				data-page-no="${page.pageNo}" data-page-size="${page.pageSize}" data-order-by="${page.orderBy}">
			<c:choose>
				<c:when test="${fns:getModule('core_extend').isEnable}">
					<div class="form-group">
						<label class="control-label">员工编码：</label>
							<form:input path="empNo" htmlEscape="false" maxlength="50" class="form-control input-sm width-90"/>
					</div>
					<div class="form-group">
					...
					<%-- <div class="form-group">
						...
						<form:select path="postCode" class="form-control width-90 input-sm"  >
					...
					<div class="form-group">
						<label class="control-label">员工姓名：</label>
							<form:input path="empName" htmlEscape="false" maxlength="50" class="form-control input-sm width-90"/>
					</div>
					<div class="form-group">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('#dataGrid').dataGrid({
			...
			// 设置数据表格列
			columnModel: [
				{header:'员工姓名', name:'empName', index:'emp_name', width:150},
				{header:'员工编码', name:'empNo', index:'emp_no', width:100},
				//<c:if test="${fns:getModule('core_extend').isEnable}">
				{header:'所属公司', name:'company.companyNameSimple', index:'c3.company_name_simple', width:160, align:"center"},//</c:if>
				{header:'组织机构', name:'office.officeName', index:'o2.office_name', width:160, align:"center"},
				{header:'电话', name:'mobile', index:'mobile', width:160, sortable:false},
				{header:'邮箱', name:'email', index:'email', width:160, sortable:false},
	...
	function selectUsers(){
		...
		$.each(dataMap, function(key, value){
			dataSize ++;
			selectHtml += '<span class="tag" id="'+key+'_tagsinput"><span>'+value.empName+'&nbsp;&nbsp;</span><a href="#" onclick="removingTag(\''+key+'\');" title="Removing tag">x</a></span>';
		});
