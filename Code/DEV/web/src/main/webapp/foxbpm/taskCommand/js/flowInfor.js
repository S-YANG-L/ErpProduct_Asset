...
function FlowInfor(config) {
	...
	function loadStatus(instanceStatus) {
		...
		if ("running" == instanceStatus) {
			...
			instanceStatus = "正常结束";
			statusEle.attr("style", "color: rgb(28, 39, 51) !important;background-color: rgb(153, 255 ,102) !important;");
		} else {
			instanceStatus = "未知状态";
			statusEle.attr("style", "color: rgb(28, 39, 51) !important;background-color: rgb(204 ,0 ,51) !important;");
		}
	...
	function loadProcessSetp(nowStep) {
		if (nowStep) {
			...
			if (taskInfo) {
				...
				var spanEle = null, imgEle = null, aEle = null, 
					users = null, type = null, id = null;
						...
						for (var m = 0, n = users.length; m < n; m++) {
							...
							if ("user" == type) {
								if ("foxbpm_all_user" == id) {
									...
									imgEle.attr("src", "images/male.png");
//									imgEle.attr("src", $this.action + "/identity/users/" + users[m].name + "/picture");
								}
							} else if ("role" == type) {
...
FlowInfor.prototype.loadProcessParticipant = function(participants) {
	...
	if (len > 0) {
		...
		participationDiv.append("<h3 class='discussion-sidebar-heading'>" + len + " 流程参与者</h3>");
		...
		for (var i = 0; i < len; i++) {
			...
			a_Img.attr("src", "images/male.png");
//			a_Img.attr("src", this.action + "/identity/users/" + participants[i].id + "/picture");
			a.append(a_Img);
			a.append(" <b>"+participants[i].name+"</b>");
