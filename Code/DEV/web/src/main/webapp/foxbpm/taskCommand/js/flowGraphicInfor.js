...
function FlowGraphicInfor(config) {
		...
		for (var i = 0; i < _self.taskListEnd.length; i++) {
			...
			for (var j = 0; j < rectAttributes.length; j++) {
				...
				if (rectAttribute.name == "stroke") {
					rectAttribute.value = backUpColorDictionary[endTask.nodeId];
				}
				if (rectAttribute.name == "stroke-width") {
					rectAttribute.value = backUpWidthDictionary[endTask.nodeId];
				}
			}
		}
	}
		...
		for (var i = 0; i < _self.taskListIng.length; i++) {
			...
			if (nodeId) {
				...
				for (var j = 0; j < rectAttributes.length; j++) {
					var rectAttribute = rectAttributes[j];
					if (rectAttribute.name == "stroke") {
						rectAttribute.value = backUpColorDictionary[ingTask.nodeId];
					}
					if (rectAttribute.name == "stroke-width") {
						rectAttribute.value = backUpWidthDictionary[ingTask.nodeId];
					}
				}
			}
		}
					...
					for (var j = 0; j < rectAttributes.length; j++) {
						...
						if (rectAttribute.name == "stroke") {
							if (!backUpColorDictionary[endTask.nodeId]) {
								backUpColorDictionary[endTask.nodeId] = rectAttribute.value;
							}
							rectAttribute.value = TASK_END_COLOR;
						}
						if (rectAttribute.name == "stroke-width") {
							if (!backUpWidthDictionary[endTask.nodeId]) {
								backUpWidthDictionary[endTask.nodeId] = rectAttribute.value;
							}
							rectAttribute.value = TASK_END_WIDTH;
						}
					...
					for (var j = 0; j < rectAttributes.length; j++) {
						...
						if (rectAttribute.name == "stroke") {
							if (!backUpColorDictionary[ingTask.nodeId]) {
								backUpColorDictionary[ingTask.nodeId] = rectAttribute.value;
							}
							rectAttribute.value = TASK_ING_COLOR;
						}
						if (rectAttribute.name == "stroke-width") {
							if (!backUpWidthDictionary[ingTask.nodeId]) {
								backUpWidthDictionary[ingTask.nodeId] = rectAttribute.value;
							}
							rectAttribute.value = TASK_ING_WIDTH;
						}
	...
	this.init = function() {
		...
		if ($this.processInstanceId != ''){
			...
			ul.append("<li><input id='yczt' type='checkbox' name='cczt' /><label for='yczt'>隐藏状态</label></li>");
		}
		if ($this.runTrackObj) {
			ul.append($("<li><input id='trackInforEle' type='checkbox' name='trackInforEle' /><label for='trackInforEle'>轨迹信息</label></li>")
							.on(
									"click",
									function() {
			...
			ul.append($("<li><input id='trackRunEle' type='checkbox' name='trackRunEle' /><label for='trackRunEle'>轨迹动态运行</label></li>")
							.on(
									"click",
									function() {
										$this.runTrackObj
