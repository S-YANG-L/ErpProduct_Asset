...
<ul>
	<c:forEach items="${page.list}" var="comment">
		<li>
			<h6>${fns:text('姓名')}: ${comment.name} &nbsp;${fns:text('时间')}：<fmt:formatDate value="${comment.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				<a href="javascript:comment(${comment.id})">${fns:text('回复')}</a></h6>
